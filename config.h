...
static char *fonts[]          = { "monospace:size=10", "NotoSans Nerd Font:size=10", "NotoColorEmoji:pixelsize=10:antialias=true:autohint=true"  };
...
static const Key keys[] = {
	...
	{ MODKEY|ShiftMask,		XK_minus,	spawn,		SHCMD("pactl set-sink-volume @DEFAULT_SINK@ -15%") },
	{ MODKEY,			XK_equal,	spawn,		SHCMD("pactl set-sink-volume @DEFAULT_SINK@ +5%") },
	{ MODKEY|ShiftMask,		XK_equal,	spawn,		SHCMD("pactl set-sink-volume @DEFAULT_SINK@ +15%") },
	...
	{ MODKEY,			XK_s,		spawn,		SHCMD("scrot -s -e 'xclip -selection clipboard -t image/png -i $f'") },
	...
	{ MODKEY, XK_v, spawn, SHCMD("choice=$(cat ~/.dmenu_paste_list | dmenu -i -p 'Paste:'); if [ -n \"$choice\" ]; then printf \"%s\" \"$choice\" | xclip -selection clipboard; printf \"%s\" \"$choice\" | xclip -selection primary; xdotool key --clearmodifiers Shift+Insert; fi") },
};
